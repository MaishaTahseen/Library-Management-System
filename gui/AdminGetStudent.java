...
public class AdminGetStudent extends JFrame implements ActionListener{
    ...
    public AdminGetStudent() {
        super("Admin Student User Control");
...
;
            if (username.isEmpty() || password.isEmpty() || email.isEmpty() ||  phone.isEmpty()) {
                JOptionPane.showMessageDialog(null,"All fields are required to add a student.");
            } else {
                Student l1 = new Student(username, password, email,phone);
            ...
            if (numberofrow >= 0) {
                ...
                String email = model.getValueAt(numberofrow, 3).toString();
                String phone = model.getValueAt(numberofrow, 4).toString();
                Student l1 = new Student(id,username, password, email, phone);
                AdminRepository ar1 = new AdminRepository();
                try {
                    ar1.removeStudent(l1);
                    model.setRowCount(0);
                    populateTable();                 
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(null, ex.getMessage());
            ...
            if (numberofrow >= 0) {
                ...
                if (name.isEmpty() || password.isEmpty() || email.isEmpty() || phn.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "All fields are required to update a student.");
                } else {
                    Student st = new Student(id,name,password,email,phn);
                    AdminRepository ar1 = new AdminRepository();
                    try {
                        ar1.updateStudent(st);
                        model.setRowCount(0);
                        populateTable();
                        }catch(Exception ex){
                            JOptionPane.showMessageDialog(null, ex.getMessage());
    ...
    private void populateTable() {
        ...
        try {
            students = ar.getStudents();
        } catch (Exception ex) { 
            JOptionPane.showMessageDialog(null, ex.getMessage());
        }
        for (int i=0; i<students.length; i++) {
        	Student student = students[i];
            String[] rowData = {Integer.toString(student.getID()),student.getUsername(), student.getPassword(), student.getMail(), student.getPhone()};
